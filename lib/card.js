...
export async function renderCard({ title = '', subtitle = '', tag = 'FIQUE POR DENTRO', variant = 0, bg = 'plain', photoUrl = '' } = {}) {
  ensureFonts();
  ...
  // se tiver foto do Pexels, usa como fundo
  if (photoUrl) {
    try {
      const photo = await loadImage(photoUrl);
      const photoW = photo.width, photoH = photo.height;
      const ratio = Math.max(S / photoW, S / photoH);
      const scaledW = photoW * ratio, scaledH = photoH * ratio;
      const offsetX = (S - scaledW) / 2, offsetY = (S - scaledH) / 2;
      ctx.drawImage(photo, offsetX, offsetY, scaledW, scaledH);
    } catch {
      // se falhar a foto, volta pro gradiente
    ...
    // fundo padrão: gradiente navy (tom de destaque varia)
    const g = ctx.createLinearGradient(0, 0, S, S);
    g.addColorStop(0, NAVY_DEEP);
    g.addColorStop(0.55, NAVY);
    g.addColorStop(1, V.g3);
  ...
  // overlay escuro (pra garantir legibilidade do texto sobre a foto)
  const ov = ctx.createLinearGradient(0, 0, 0, S);
  ov.addColorStop(0, `rgba(20,30,50,0.65)`);
  ov.addColorStop(0.5, `rgba(20,30,50,0.55)`);
  ov.addColorStop(1, `rgba(20,30,50,0.70)`);
  ctx.fillStyle = ov;
  ctx.fillRect(0, 0, S, S);
  ...
  // fundo temático só se não tiver foto (bem sutil, "apagado pelo azul")
  if (!photoUrl) drawTheme(ctx, S, V, bg);
...
// Gera a arte vertical para Stories (1080x1920)
export async function renderStory({ title = '', subtitle = '', tag = 'FIQUE POR DENTRO', variant = 0, bg = 'plain', photoUrl = '' } = {}) {
  ensureFonts();
  const logo = await ensureLogo();
  const V = VARIANTS[((variant % VARIANTS.length) + VARIANTS.length) % VARIANTS.length];
  const W = 1080, H = 1920;
  ...
  // fundo com foto ou gradiente
  if (photoUrl) {
    try {
      const photo = await loadImage(photoUrl);
      const photoW = photo.width, photoH = photo.height;
      const ratio = Math.max(W / photoW, H / photoH);
      const scaledW = photoW * ratio, scaledH = photoH * ratio;
      const offsetX = (W - scaledW) / 2, offsetY = (H - scaledH) / 2;
      ctx.drawImage(photo, offsetX, offsetY, scaledW, scaledH);
    } catch {
  ...
  // glow
  const rg = ctx.createRadialGradient(W * 0.85, H * 0.12, 0, W * 0.85, H * 0.12, 800);
  rg.addColorStop(0, `rgba(${V.glow},0.35)`);
  rg.addColorStop(1, `rgba(${V.glow},0)`);
  ctx.fillStyle = rg;
  ctx.fillRect(0, 0, W, H);
  ...
  // tema sutil
  if (!photoUrl) drawThemeStory(ctx, W, H, V, bg);

  const PAD = 80;

  // logo pequeno mais para baixo (evita ser coberta pelo logo do Stories)
  const logoH = 100;
  const logoW = logo.width * (logoH / logo.height);
  ctx.drawImage(logo, PAD, 200, logoW, logoH);
  ...
  // manchete grande centralizada verticalmente
  const maxW = W - PAD * 2;
  let size = 64, lines = [];
  while (size >= 36) {
    ctx.font = `700 ${size}px "Poppins Bold"`;
  ...
  ctx.font = `700 ${size}px "Poppins Bold"`;
  ctx.fillStyle = '#ffffff';
  const titleTop = 700; // muito mais para o meio da story
  lines.forEach((ln, i) => ctx.fillText(ln, PAD, titleTop + i * lh + size * 0.80));
  const titleBottom = titleTop + lines.length * lh;

  // subtitle
  if (subtitle && subtitle.trim().length > 0) {
    ctx.font = '400 32px "Poppins Regular"';
    ctx.fillStyle = 'rgba(255,255,255,0.82)';
    const subTop = titleBottom + 40;
    const subLines = wrap(ctx, subtitle.trim(), maxW);
    const maxSubLines = 3;
    const fit = Math.min(maxSubLines, subLines.length);
    subLines.slice(0, fit).forEach((ln, i) => {
      ctx.fillText(ln, PAD, subTop + i * 48 + 32 * 0.80);
    });
  }
  ...
  ctx.fillText('Saiba mais no nosso site', PAD, H - 160);
  ctx.fillStyle = BLUE;
  ctx.font = '700 28px "Poppins Bold"';
  ctx.fillText('tatianefontes.com', PAD, H - 120);

  ctx.fillStyle = 'rgba(255,255,255,0.45)';
  ctx.font = '400 18px "Poppins Regular"';
  ctx.textAlign = 'right';
  ctx.fillText('Fonte: Portal Contábeis', W - PAD, H - 80);
  ctx.textAlign = 'left';

  return canvas.toBuffer('image/jpeg', 92);
}

// fundos temáticos para stories (1080x1920)
function drawThemeStory(ctx, W, H, V, bg) {
  ...
  if (bg === 'grid') {
    ...
    for (let y = 0; y <= H; y += 66) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
  } else if (bg === 'dots') {
    ctx.fillStyle = 'rgba(255,255,255,0.05)';
    for (let y = 46; y < H; y += 56) for (let x = 46; x < W; x += 56) { ctx.beginPath(); ctx.arc(x, y, 3, 0, 7); ctx.fill(); }
  } else if (bg === 'chart') {
    const pts = [[0, H * 0.80], [W * 0.22, H * 0.72], [W * 0.42, H * 0.76], [W * 0.62, H * 0.60], [W * 0.82, H * 0.64], [W, H * 0.46]];
    const path = () => {
    ...
    const n = 6, bw = 78, gap = 34; let x = W - (n * (bw + gap)) + gap - 40;
    ctx.fillStyle = `rgba(${V.glow},0.12)`;
    for (let i = 0; i < n; i++) { const h = 110 + i * 78; roundRect(ctx, x, H - h - 40, bw, h, 14); ctx.fill(); x += bw + gap; }
  } else if (bg === 'coins') {
    ctx.strokeStyle = 'rgba(255,255,255,0.06)';
    const circ = [[W * 0.82, H * 0.30, 150], [W * 0.90, H * 0.62, 90], [W * 0.70, H * 0.52, 54]];
